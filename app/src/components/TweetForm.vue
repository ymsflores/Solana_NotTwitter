...
const characterLimit = useCountCharacterLimit(content, 250);
...
<template>
  <div v-if="connected" class="px-8 py-4 border-b border-slate-800">
    <textarea
      ref="textarea"
      rows="1"
    ...
    <div class="flex flex-wrap items-center justify-between -m-2">
      <div class="relative m-2 mr-4">
        <input
          type="text"
          placeholder="topic"
          class="text-sky-500 rounded-full pl-10 pr-4 py-2 bg-transparent"
        ...
        <div class="absolute left-0 inset-y-0 flex pl-3 pr-2">
          <svg
            ...
            :class="effectiveTopic ? 'text-sky-500' : 'text-gray-400'"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
      ...
      <div class="flex items-center space-x-6 m-2 ml-auto">
        ...
        <button
          ...
          :disabled="!canTweet"
          :class="canTweet ? 'bg-sky-500' : 'bg-sky-950 cursor-not-allowed'"
          @click="send"
        >
          Tweet
        </button>
